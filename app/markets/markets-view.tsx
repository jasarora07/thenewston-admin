...
// Performance: Lazy load the heavy chart to boost speed scores
const AdvancedChart = dynamic(() => import("@/components/advanced-chart"), {
  ...
  loading: () => (
    <div className="h-full w-full flex flex-col items-center justify-center bg-[#131722] gap-4">
      <div className="h-8 w-8 border-2 border-[#22c55e] border-t-transparent rounded-full animate-spin" />
    ...
    <div className="min-h-screen bg-black text-foreground flex flex-col">
      <main className="flex-1 container mx-auto px-4 py-8 space-y-8">
        <header className="border-b border-white/10 pb-6 text-left">
          <h1 className="text-3xl font-black tracking-tighter flex items-center gap-3 italic text-white uppercase">
            Western <span className="text-[#22c55e] italic">Terminal</span>
          </h1>
        </header>

        {/* SELECTORS */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-zinc-900/20 p-6 rounded-2xl border border-white/5">
          <div className="space-y-2">
            ...
            <div className="relative">
              <select 
                ...
                className="w-full h-12 bg-black border border-white/10 rounded-lg px-4 text-sm font-semibold text-white appearance-none cursor-pointer focus:ring-1 focus:ring-[#22c55e] outline-none"
              >
                {Object.keys(MARKET_DATA).map(r => <option key={r} value={r}>{r}</option>)}
              </select>
              <ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 h-4 w-4 pointer-events-none text-zinc-500" />
            </div>
          </div>
        ...
        {/* CHART */}
        <div className="rounded-xl border border-white/5 bg-[#131722] overflow-hidden h-[650px] shadow-2xl relative z-10">
