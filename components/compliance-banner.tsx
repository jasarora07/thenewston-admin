...
export function ComplianceBanner() {
  ...
  useEffect(() => {
    ...
    // 1. Safely check the browser's User-Agent string signature
    const userAgent = typeof window !== "undefined" && window.navigator?.userAgent 
      ? window.navigator.userAgent.toLowerCase() 
      : ""
    
    // 2. Identify automated web crawlers, search index bots, and headless testers
    ...
    if (!isBot) {
      ...
      // Keep it hidden for bots to prevent asset stream loading loops
      setIsVisible(false)
    }
  }, [])
